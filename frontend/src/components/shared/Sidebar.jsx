...
const NAV = [
  ...
  { to: '/invoices', label: 'Facturas', icon: 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z', badge: 'overdue' },
  { to: '/galleries', label: 'Galerías', icon: 'M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z' },
]
  ...
  return (
    <aside className="w-64 bg-zinc-900 border-r border-zinc-800 flex flex-col shrink-0">
      <div className="p-6 border-b border-zinc-800">
        ...
        <p className="text-xs text-zinc-500 mt-0.5">Panel del fotógrafo</p>
      </div>

      <nav className="flex-1 p-4 space-y-1">
        {NAV.map((item) => {
          const overdueCount = item.badge === 'overdue' ? (stats?.overdue_invoices ?? 0) : 0
          return (
            <NavLink
              key={item.to}
              to={item.to}
      ...
      <div className="p-4 border-t border-zinc-800">
        <NavLink
          to="/profile"
          className={({ isActive }) =>
            `flex items-center gap-3 px-3 py-2 mb-1 rounded-lg transition-all ${isActive ? 'bg-white/10' : 'hover:bg-zinc-800'}`
          }
        >
          <div className="h-8 w-8 rounded-full bg-zinc-700 flex items-center justify-center text-sm font-medium text-zinc-300 shrink-0">
